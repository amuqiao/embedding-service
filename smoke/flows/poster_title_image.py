    ...
    *,
    ...
    if storage_backend == "aliyun_oss":
        ...
        upload = oss_image_upload.upload_image(
            ...
            content_type=reference_content_type,
            app_env=app_env,
            key_prefix="smoke/poster-title-image/reference",
        )
...
def build_job_payload(
    *,
    ...
    return {
        "client_request_id": client_request_id or f"smoke-poster-title-image-{uuid.uuid4()}",
        "job_type": DEFAULT_JOB_TYPE,
        "job_params": {"items": items},
        "metadata": {"source": "scripts/smoke.sh poster-title-image"},
...
def _download_url(url: str, *, timeout_seconds: int = 30) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "smoke.sh/1.0"})
    try:
...
def run(
    *,
    ...
    if json_output:
        ...
        formatters.section("Poster Title Image Smoke")
