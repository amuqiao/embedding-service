...
if [[ -z "${RELEASE_TEST_BOOTSTRAPPED:-}" && -f "${BASH_SOURCE[0]}" ]]; then
  tmp_script="${TMPDIR:-/tmp}/release-test.$$.$RANDOM.sh"
  cp "${BASH_SOURCE[0]}" "$tmp_script"
  chmod +x "$tmp_script"
  RELEASE_TEST_BOOTSTRAPPED=1 exec bash "$tmp_script" "$@"
fi
...
# 命令速查
#
# 这几条命令都在主项目 dev 目录执行。脚本内部会进入 tmp 发布目录处理 test。
#
# 推荐流程：
#   1. ./scripts/deploy/release-test.sh          # dev 目录发起；tmp 仓库准备 test 合并
#   2. ./scripts/deploy/release-test.sh status   # 同时查看 dev 目录和 tmp 发布目录
#   3. ./scripts/deploy/release-test.sh --push   # dev 目录发起；tmp 仓库提交并推送 test
#
# 所有命令：
#   ./scripts/deploy/release-test.sh             # 等同于 prepare
#   ./scripts/deploy/release-test.sh prepare     # 准备测试发布
#   ./scripts/deploy/release-test.sh --push      # 提交并推送，推荐第二次执行使用
#   ./scripts/deploy/release-test.sh push        # --push 的等价写法
#   ./scripts/deploy/release-test.sh -p          # --push 的短选项
#   ./scripts/deploy/release-test.sh status      # 查看状态和发布判断
#   ./scripts/deploy/release-test.sh --help      # 查看完整帮助
#
...
#
# 测试环境发布链路：
#   dev 代码 -> 合入 test -> CI 构建镜像 -> Kuboard 手动切镜像 -> 验证服务
#
# 本脚本只负责前两步 Git 流程。它不负责 Kuboard 切镜像，也不判断业务功能
# 是否正确。为避免影响日常 dev 工作区，脚本会在 RELEASE_TMP_DIR 指向的
# tmp 发布目录重建独立 Git 仓库，再在该仓库切换 test、merge 和 push。tmp 发布目录出问题
# 可以删除重建；主项目目录不会被切换到 test，也不会进入 merge 状态。
#
# 目录职责：
# - 主项目目录：只允许在 dev 分支执行；只做状态检查和来源分支发布前确认。
# - tmp 发布目录：允许切换 test、merge origin/dev、commit 和 push origin test。
#
# 文件分层：
# - 发版控制文件：.gitlab-ci.yml、Dockerfile、Dockerfile_OS
#   决定 CI 和镜像构建方式，脚本默认保留 test 分支版本。
# - 构建风险文件：.dockerignore、.gitignore、pyproject.toml、uv.lock、.python-version
#   变更可能影响 Docker 构建上下文、依赖安装、Python 版本或 Git 跟踪范围。
# - 业务代码文件：main.py、detectors/*、config.py、auth.py 等
#   通常不影响脚本流程，但会影响服务启动、接口行为和发布后验证。
###############################################################################
...
# 示例：
#   SOURCE_REF=origin/feature-x ./scripts/deploy/release-test.sh
#   BUILD_MODE=os ./scripts/deploy/release-test.sh --push   # 通常仅运维需要
#   COMMIT_MESSAGE="[ci build] release supported languages to test" ./scripts/deploy/release-test.sh --push
###############################################################################
...
# tmp 发布目录。默认放在当前仓库同级 tmp 目录下，避免影响日常 dev 工作区。
# 未显式设置时，根据当前仓库目录名生成：../tmp/<repo-name>-release-test。
...
# 二者都属于 test 分支发布配置，不用 dev 分支版本覆盖。
...
usage() {
  cat <<'EOF'
用法：
  ./scripts/deploy/release-test.sh [prepare|push|--push|status]

执行位置：
  建议始终在主项目 dev 目录执行。本脚本会自动进入 tmp 发布目录处理 test。

默认动作：
  prepare

动作说明：
  prepare  检查主 dev，重建 tmp 仓库，在 tmp 仓库切到 test 并合入 origin/dev。
  push     进入 tmp 仓库，提交 prepare 产生的待发布 merge，并推送到 origin/test 触发 CI。
  --push   push 的别名，适合第二次执行时直接添加选项。
  status   同时输出主 dev 目录、tmp 发布目录状态，并给出 prepare/push 判断。

关键配置：
  SOURCE_REF       要合入测试环境的来源分支，默认：origin/dev
  TARGET_BRANCH   测试环境发版分支，默认：test
  RELEASE_TMP_DIR  tmp 发布目录，默认：../tmp/<repo-name>-release-test
  BUILD_MODE      构建方式，ci 或 os，默认：ci
                   ci -> [ci build]，生成代码镜像，开发默认使用
                   os -> [build:os]，生成运行环境镜像，通常由运维使用
  PROJECT_NAME    Kuboard 容器提示使用的项目名，默认：当前仓库目录名
  COMMIT_MESSAGE  push 阶段使用的提交信息，必须包含对应构建标记。

脚本内维护的文件分组：
  PROTECTED_FILES 合并时自动保留 test 版本的发版控制文件。
  RISK_FILES      变更后只提示人工确认的构建风险文件。

示例：
  ./scripts/deploy/release-test.sh
  ./scripts/deploy/release-test.sh prepare
  ./scripts/deploy/release-test.sh push
  ./scripts/deploy/release-test.sh --push
  BUILD_MODE=os ./scripts/deploy/release-test.sh --push
  COMMIT_MESSAGE="[ci build] release supported languages to test" ./scripts/deploy/release-test.sh --push
EOF
}
...
warn() {
  printf '[release-test] 告警：%s\n' "$*" >&2
}

next_step() {
  printf '[release-test] 下一步：%s\n' "$*"
}

ok() {
  printf '[release-test] OK：%s\n' "$*"
}
...
release_tmp_path() {
  local release_dir
  release_dir="${RELEASE_TMP_DIR:-../tmp/$(basename "$(pwd)")-release-test}"
...
ensure_main_ready_for_prepare() {
  ...
  log_boundary "检查主项目 dev 目录"
  log "当前执行目录：$(pwd)"
  log "当前分支：${branch}"
  log "说明：主项目目录只做检查和来源分支发布前确认，不会切到 ${TARGET_BRANCH}。"

  [[ "$branch" == "dev" ]] ||
    stop_with_next_step "当前主开发目录必须在 dev 分支，实际是：${branch}" "切回 dev 后重新执行：git switch dev && ./scripts/deploy/release-test.sh"

  if [[ -n "$(git status --porcelain)" ]]; then
    warn "主开发工作区不干净，prepare 不会继续执行。"
    git status --short
    next_step "先提交或清理 dev 工作区改动，然后重新执行：./scripts/deploy/release-test.sh"
    exit 1
  fi
  ...
  if [[ "$SOURCE_REF" == "origin/dev" ]] &&
    ...
    if [[ "$ahead" != "0" ]]; then
      stop_with_next_step "本地 dev 领先 origin/dev ${ahead} 个提交；脚本默认发布 origin/dev，不会包含这些本地提交。" "先执行：git push origin dev；然后重新执行：./scripts/deploy/release-test.sh"
    fi
    if [[ "$behind" != "0" ]]; then
      stop_with_next_step "本地 dev 落后 origin/dev ${behind} 个提交；脚本不会在旧 dev 状态下发布。" "先同步 dev 后重新执行，例如：git pull --ff-only origin dev"
    fi
  fi
}
...
release_tmp_source_meta() {
  git rev-parse --git-path release-test-source-sha
}
...
main_readiness_reason() {
  ...
  branch="$(git branch --show-current)"
  if [[ "$branch" != "dev" ]]; then
    printf '主项目目录必须在 dev 分支，当前是 %s' "$branch"
    return
  fi

  if [[ -n "$(git status --porcelain)" ]]; then
    printf '主项目 dev 工作区不干净'
    return
  fi
  ...
  if [[ "$SOURCE_REF" == "origin/dev" ]] &&
    ...
    behind="$(git rev-list --count dev..origin/dev)"
    if [[ "$ahead" != "0" ]]; then
      printf '本地 dev 领先 origin/dev %s 个提交，需要先 git push origin dev' "$ahead"
      return
    fi
    if [[ "$behind" != "0" ]]; then
      printf '本地 dev 落后 origin/dev %s 个提交，需要先同步 dev' "$behind"
      return
    fi
  fi
}
...
assert_release_tmp_safe() {
  ...
  [[ "$release_base" == *"release-test"* ]] || die "tmp 发布目录名称必须包含 release-test：${release_root}"
...
prepare_release_copy() {
  ...
  if [[ -e "$release_root" ]]; then
    if git -C "$release_root" rev-parse --show-toplevel >/dev/null 2>&1 &&
      ...
      stop_with_next_step "tmp 发布目录已经存在一个未完成的 merge，prepare 不会覆盖。" "先检查 tmp 发布目录；确认无误后执行：./scripts/deploy/release-test.sh --push"
    fi
...
enter_existing_release_copy() {
  ...
  release_root="$(release_tmp_path)"
  [[ -e "$release_root" ]] ||
    stop_with_next_step "tmp 发布目录不存在：${release_root}" "先执行默认准备流程：./scripts/deploy/release-test.sh"
  ...
  log "当前目录：$(pwd)"
  log "说明：主项目 dev 目录不会被提交、切分支或推送 test。"
}
...
warn_risk_files_changed() {
  local changed
  changed="$(git diff --name-only HEAD -- "${RISK_FILES[@]}" || true)"

  if [[ -n "$changed" ]]; then
    log "注意：以下构建风险文件在本次发布中发生变化，请确认符合预期："
    printf '%s\n' "$changed" | sed 's/^/[release-test]   - /'
    log "这些文件可能影响 Docker 构建上下文、依赖安装、Python 版本或 Git 跟踪范围。"
  fi
}
...
show_release_decision() {
  ...
  if [[ "$tmp_exists" != "yes" ]]; then
    ...
    if [[ -n "$main_reason" ]]; then
      next_step "先处理 prepare 不可执行的原因，然后重新执行：./scripts/deploy/release-test.sh status"
    else
      next_step "执行默认准备流程：./scripts/deploy/release-test.sh"
    fi
    return
  fi
  ...
  if [[ "$tmp_branch" != "$TARGET_BRANCH" ]]; then
    ...
    next_step "重新执行准备流程：./scripts/deploy/release-test.sh"
    return
  fi

  if [[ "$tmp_has_conflicts" == "yes" ]]; then
    warn "push：不可执行，原因：tmp 发布目录仍有未解决冲突"
    next_step "进入 tmp 发布目录解决冲突并 git add，然后执行：./scripts/deploy/release-test.sh --push"
    return
  fi

  if [[ "$tmp_has_merge" == "yes" ]]; then
    ok "push：可以执行"
    next_step "检查 tmp 发布目录 diff 后执行：./scripts/deploy/release-test.sh --push"
    return
  fi

  if [[ "$tmp_has_changes" == "yes" ]]; then
    warn "push：不可执行，原因：tmp 发布目录有变更，但不是 prepare 产生的待发布 merge"
    next_step "如确认废弃本次发布，可删除 tmp 发布目录后重新执行：./scripts/deploy/release-test.sh"
    return
  fi
  ...
  if [[ -n "$main_reason" ]]; then
    ...
    next_step "如需发布，先执行默认准备流程：./scripts/deploy/release-test.sh"
  fi
}
...
prepare_release() {
  ...
  if [[ -f "$(git rev-parse --git-path MERGE_HEAD)" ]]; then
    show_release_decision
    stop_with_next_step "tmp 发布目录已经存在一个未完成的 merge，prepare 不会重复执行。" "先检查 tmp 发布目录；确认无误后执行：./scripts/deploy/release-test.sh --push"
  fi
  ...
  if [[ -n "$(git status --porcelain)" ]]; then
    ...
    git status --short
    show_release_decision
    next_step "确认废弃本次发布时，可删除 tmp 发布目录后重新执行：./scripts/deploy/release-test.sh"
    exit 1
  fi
  ...
  if ! git merge --no-commit --no-ff "${SOURCE_REF}"; then
    ...
    if [[ -z "$(git diff --name-only --diff-filter=U)" ]]; then
      ...
      show_release_status
      show_release_decision
      next_step "./scripts/deploy/release-test.sh --push"
      exit 0
    fi
    git status --short
    show_release_decision
    stop_with_next_step "检测到合并冲突，脚本已停止在待人工处理状态。" "解决业务代码冲突并 git add 后，执行：./scripts/deploy/release-test.sh --push"
  fi
  ...
  show_release_decision
  log "建议检查命令："
  log "  ./scripts/deploy/release-test.sh status"
  log "  cd $(pwd)"
  log "  git diff --cached"
  log "  cd ${MAIN_REPO_ROOT}"
  log "下一步："
  log "  ./scripts/deploy/release-test.sh --push"
}
...
push_release() {
  ...
  [[ "$branch" == "${TARGET_BRANCH}" ]] || {
    show_release_decision
    stop_with_next_step "当前分支必须是 ${TARGET_BRANCH}，实际是：${branch}" "先执行默认准备流程：./scripts/deploy/release-test.sh"
  }
  [[ -f "$(git rev-parse --git-path MERGE_HEAD)" ]] || {
    show_release_decision
    stop_with_next_step "没有检测到待提交的 merge，不能直接 push。" "先执行默认准备流程：./scripts/deploy/release-test.sh"
  }

  if [[ -n "$(git diff --name-only --diff-filter=U)" ]]; then
    git status --short
    show_release_decision
    stop_with_next_step "仍有未解决的合并冲突。" "解决冲突并 git add 后，再执行：./scripts/deploy/release-test.sh --push"
  fi
  ...
  if [[ -f "$meta_file" ]]; then
    ...
    [[ "$prepared_sha" == "$current_sha" ]] ||
      {
        show_release_decision
        stop_with_next_step "${SOURCE_REF} 在 prepare 后发生变化，当前 tmp 发布内容可能不是最新代码。" "重新执行：./scripts/deploy/release-test.sh"
      }
  else
  ...
  if [[ -n "$(git ls-files --others --exclude-standard)" ]]; then
    git status --short
    show_release_decision
    stop_with_next_step "存在未跟踪文件，脚本不会自动把它们带入发布提交。" "确认这些文件后，添加、删除或加入 .gitignore，再执行：./scripts/deploy/release-test.sh --push"
  fi
  ...
  log "push 完成；请等待钉钉 CI 镜像通知，然后到 Kuboard 更新镜像版本"
  local project
  project="$(project_name)"
  if [[ "$BUILD_MODE" == "ci" ]]; then
    next_step "拿到代码镜像版本后，在 Kuboard 调整“初始化容器 test-${project}”的新版本；不要改工作容器 OS 镜像。"
  else
    next_step "拿到运行环境镜像版本后，由运维确认是否调整“工作容器 os-${project}”的 OS-* 新版本。"
  fi
}
...
status_release() {
  ensure_main_repo
  ensure_build_mode
  log_boundary "主项目 dev 目录状态"
  log "说明：这里是日常开发目录，只检查，不做 test 分支发布操作。"
  show_release_status
  ...
  if [[ -e "$release_root" ]] && git -C "$release_root" rev-parse --show-toplevel >/dev/null 2>&1; then
    ...
    log "说明：这里才是 test 分支合并、提交、推送发生的地方。"
    show_release_status
  else
    printf '\n'
