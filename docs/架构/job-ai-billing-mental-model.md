...
- `model_id`、runtime snapshot、AI call ledger 和 Billing read model 分别保存什么事实。
- 为什么 AI call ledger 不应强绑定 `job_id`。
...
- `ai_call_ledger_entries` 记录每次真实 AI provider 调用的 ledger、usage、cost estimate 和 scope 归属；ORM 类名当前仍为 `AiCallLog`。
...
AI gateway
  ...
  ├─ 写入 ai_call_ledger_entries
  └─ 返回模型输出、usage 或稳定错误
        ↓
Billing read model
  └─ 按 scope_type / scope_id 聚合 ai_call_ledger_entries
```
...
- `ai_call_ledger_entries` 是 AI provider call 明细账本，不是 `job_aggregates` 表的扩展字段集合。
...
| 对象 | 回答的问题 | 事实源 | 对外形态 |
|---|---|---|---|
| `Job` | 这次异步执行现在是什么状态，公开结果或公开错误是什么 | `job_aggregates`、`job_execution_attempts`、`dispatch_outbox`、`callback_outbox`、`job_audit_events` | `JobEnvelope` |
| AI gateway | 这次模型调用应该使用哪个服务模型、provider model，并返回什么模型输出和 usage | `model catalog`、LiteLLM adapter、provider response | 上层调用方的执行结果 |
| Billing | 哪些模型调用发生了，usage 是多少，按哪个价格版本估算了多少成本 | `ai_call_ledger_entries`、pricing snapshot | scope billing、Job billing、内部明细查询或导出 |

这三类事实不能混成一张表或一个公共外壳。

```text
JobEnvelope 不保存 usage/cost 明细。
ai_call_ledger_entries 不保存 Job 当前状态。
...
Job 相关表只保存平台执行状态、执行权、投递意图和对外投影：
...
job_execution_attempts
  保存一次执行尝试的领取、租约、heartbeat、终态和错误。

dispatch_outbox
  保存 Taskiq publish 意图、发布重试和应用级 dead letter。
...
AI gateway facade
  ↓ 调用时审计复制
ai_call_ledger_entries.model_id / provider / provider_model / pricing_ref
```

因此，`job_aggregates` 表不需要顶层 `model_id`。如果某个 Job 不调用模型，`runtime_fields.model_id` 可以不存在；如果某个 Job 调用多个模型，每次调用也应分别进入 `ai_call_ledger_entries`。
...
scope billing
  按 scope_type / scope_id 聚合 ai_call_ledger_entries 得到的成本估算返回对象。
...
当前已新增 `ai_call_ledger_entries` 作为 AI call ledger。当前不需要新增 `job_billing_summaries` 事实表；如果后续读压或导出要求需要投影表，它也只能是 `ai_call_ledger_entries` 的派生读模型。
...
- Billing service 按 scope 聚合 `ai_call_ledger_entries`。

不允许：

- AI gateway 直接修改 `job_aggregates.status`。
- AI gateway 直接创建或投递 Callback。
- Billing service 直接改变 Job succeeded / failed。
- Job 状态机依赖 `cost_amount` 是否存在来判断执行成功。
- `job_aggregates` 表直接保存 provider usage / cost 明细。
...
后续如果新增同步 AI 能力接口，应让该接口复用 AI gateway 和 `ai_call_ledger_entries`：
...
```text
scope_type + scope_id + ai_call_ledger_entries
```
...
- 每次真实调用的模型审计事实应进入 `ai_call_ledger_entries`。
...
抽象上，计费服务依赖的是“可计量调用事实”，不是 AI 这个业务名词。未来如果出现非 AI 的可计量能力，应单独设计通用 metered usage，而不是提前把首版 `ai_call_ledger_entries` 泛化成过宽的表。
...
Job:
  ...
  [ ] 是否避免把 provider usage / cost 明细塞进 `job_aggregates`。
...
Usage ledger:
  ...
  [ ] 每次真实 provider 调用是否都有独立 ai_call_ledger_entries。
  [ ] ai_call_ledger_entries 是否使用 scope_type / scope_id 表达归属。
  [ ] 缺 usage / 缺价格 / 成本计算失败是否 fail-fast。

Billing:
  [ ] Billing read model 是否只从 ai_call_ledger_entries 聚合。
