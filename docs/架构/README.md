...
| [新增 job_type 标准接入规范](job-type-extension-standard.md) | 接入流程：新增 `job_type` 时如何补 Params、Runtime、Result、WorkflowHandler、错误、日志和测试。 | 具体 Prompt、模型输出内容、业务标签体系。 |
...
- `job_type` registry、WorkflowHandler metadata、能力 schema、Job 错误或能力测试规则变化时，应同步更新 [新增 job_type 标准接入规范](job-type-extension-standard.md)。
