...
本文定义新增 `job_type` 的固定接入路径：一个能力必须同时声明 schema、runtime、执行计划、结果投影、错误、日志和验证。
...
- 如何通过 `WorkflowHandler` 和 registry checks 支持横向扩展。
...
`job_type` 只定义能力自己的输入和输出；通用 Job 外壳只能复用一次公共定义。
...
app/schemas/callbacks.py     CallbackEnvelope
app/core/workflow_registry.py WorkflowHandler、JobTypeSpec、执行侧 registry 实现
app/jobs/registry.py         job_type 注册公开入口
...
app/workflows/<job_type>.py
  WorkflowHandler 子类

app/workflows/register.py
  显式注册 handler
...
3. 定义 `CanonicalResultSchema`。
   ...
   canonical result 是内部事实源，用于恢复、合并、副作用和最终投影。模型原始输出、分片中间结果和公开结果不能互相替代。
...
6. 实现执行计划。
   ...
   简单能力实现 `build_execution_plan()` 和 `execute_standard_item()`。需要模型文本运行时的能力可以返回 `None` 使用通用执行器，但必须补齐 prompt、模型和 output contract 校验。

7. 注册 handler。

   在 `app/workflows/register.py` 中通过 `app/jobs/registry.py` 显式注册。注册入口应幂等，registry 本身不允许静默覆盖重复 `job_type`。
...
[ ] WorkflowHandler 声明完整 JobTypeSpec metadata
[ ] runtime_job_fields() 返回值符合 RuntimeFieldsSchema
[ ] build_execution_plan() 或通用 planner 路径明确
[ ] execute_standard_item() / parse_output() / merge_chunks() 路径明确
[ ] allow_callback 策略明确
[ ] large_artifact_keys 策略明确
[ ] allowed_error_codes 全部已注册
[ ] log_events 全部已注册
[ ] register_all_workflows() 显式注册且保持幂等
[ ] 参数校验测试通过
...
新增能力时优先横向增加 handler、schema 和测试，不修改通用 Job 外壳。只有当多个 `job_type` 出现重复字段、重复执行路径或重复结果投影规则时，才考虑抽公共模块。
