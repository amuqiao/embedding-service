...
| 类型 | 当前例子 | 规则 |
|---|---|---|
| 纯 shell 手写 help | `dev.sh`、`verify.sh`、`deploy.sh`、`k8s.sh`、`tools.sh` | 可以手写 `用法`、`命令`、`选项` 和说明区块。 |
| shell wrapper + Typer CLI | `jobs.sh`、`job-ops.sh`、`real-flow.sh` | Typer 自动生成的 `Usage`、`Options`、`Commands` 是命令和参数事实源；手写 epilog 不能再重复一份完整命令目录。 |
| shell wrapper + 普通 Python / argparse | `tools.sh env-url` 这类下沉实现 | 参数事实源以实际执行层为准；wrapper 只在需要统一入口体验时手写薄 help。 |
| release helper | `deploy/release-test.sh`、`deploy/release-master.sh` | 不属于 `scripts/`，但复用本合同的 envelope 结构。 |
...
- 手写区不能再出现重复命令目录。
- `jobs.sh`、`real-flow.sh` 等入口的子命令 help 必须保持可访问。
