...
| shell wrapper + Typer CLI | `jobs.sh`、`redis.sh`、`oss.sh`、`job-ops.sh`、`load.sh` | Typer 自动生成的 `Usage`、`Options`、`Commands` 是命令和参数事实源；手写 epilog 不能再重复一份完整命令目录。 |
...
| shell wrapper + 普通 Python / argparse | `ai.sh`、`tools.sh env-url` 这类下沉实现 | 参数事实源以实际执行层为准；wrapper 只在需要统一入口体验时手写薄 help。 |
