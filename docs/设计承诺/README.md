...
| 文档 | 用途 | 受众 |
|---|---|---|
| **AI能力层API稳定性承诺.md** | 明确"永远稳定"的 API 层与"可自由变化"的内部实现的分界线。包含版本管理规则、扩展示例 | PO、架构师 |
| **Work_note与优化建议的角色.md** | 深度解释 `work_note` prompt block 的两个不同角色和 `optimization_prompt` 的使用方式。含完整代码示例和最佳实践 | 后端开发者 |
