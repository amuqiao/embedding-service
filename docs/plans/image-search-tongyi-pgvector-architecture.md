...
- 图片打标独立计划：[`asset-image-tagging-job-architecture.md`](asset-image-tagging-job-architecture.md)
...
- 不替代图片打标 Job；打标产出的描述或标签只有在后端确认为 `text_payload` 后，才进入本 capability。
...
| AI 描述生成和打标 | 不覆盖 | 独立图片打标或描述生成 Job |

因此，本方案能覆盖“后端资源进入向量库、被搜索、被更新、被删除、被对账修复、被全量重建”的服务侧闭环。它不覆盖资源管理产品本身，也不覆盖前端交互和 AI 描述生成。
...
| 全部资源组默认生成 AI 描述 | 不属于本服务 | 属于图片打标/描述生成或后端资源流程 |
| AI 描述中英双版本 | 不属于本服务 | 后端可把中英文本拼入 `text_payload` |
| 身体不需要生成描述 | 不属于本服务 | 后端决定是否提交 `text_payload`；本服务只生成 `asset` 主向量 |
...
| 脸/头发/物件/背景/BGM/音效描述模板 | 不覆盖模板生成 | 属于图片打标/描述生成计划，不属于向量检索能力 |
...
```text
本文覆盖 4.6 的向量检索服务侧能力。
本文不覆盖 4.6 的前端交互、资源管理、收藏、普通搜索和描述生成模板。
```
...
不建议当前直接做全自动“打标 + 索引 + 搜索”闭环。图片打标是独立计划，描述和标签进入搜索文本前应由后端确认。
