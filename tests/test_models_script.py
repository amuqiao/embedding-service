...
def test_models_verify_fails_when_required_files_are_missing(tmp_path):
    ...
    assert "required model files are missing or empty" in result.stderr
...
def test_models_download_uses_hf_and_checks_downloaded_files(tmp_path):
    ...
    fake_hf.write_text(
        ...
        "done\n"
        "mkdir -p \"$local_dir\"\n"
        "for name in htdemucs_ft_drums.onnx htdemucs_ft_bass.onnx htdemucs_ft_other.onnx htdemucs_ft_vocals.onnx bag_infer.py requirements.txt; do\n"
        "  printf '%s\\n' \"$name\" > \"$local_dir/$name\"\n"
        "done\n",
        encoding="utf-8",
    )
