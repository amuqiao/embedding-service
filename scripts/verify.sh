...
配置与环境变量：
  ...
  ENABLED_BUSINESS_PACKAGES  test/check 会忽略该值，按全部静态注册业务包执行本地验证。
...
作用域：
  运行当前仓库 pytest。
  pytest 默认验证全部静态注册业务包，不继承当前服务实例的 ENABLED_BUSINESS_PACKAGES 启用子集。
...
作用域：
  ...
  registry consistency 和 pytest 阶段默认验证全部静态注册业务包，不继承当前服务实例的 ENABLED_BUSINESS_PACKAGES 启用子集。
