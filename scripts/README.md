...
- 顶层 `-h` 只放基础用法和少量最短示例。
- 多子命令入口可以提供无参默认的只读概览，例如 `jobs.sh` 的 overview；`-h` 仍负责命令帮助。
- 每个子命令都必须支持 `-h`，并给出该子命令可复制修改的典型示例。
- 顶层 `-h` 应覆盖最常用的 3 到 6 条入口路径，不要求每个子命令都出现。
...
示例不是命令清单，而是帮助用户判断“现在该复制哪条命令”。好的示例应当可以直接复制粘贴，只需修改 `job_id`、路径、时间窗口、模型、环境文件等关键参数就能使用，减少读者先理解全部参数再自行拼命令的成本。新增或调整 help 示例时，先回答三问：

```text
第一眼入口：用户只看顶层 -h，能否复制一条命令完成最常见动作？
具体配置：用户进入子命令 -h，能否看到该命令最典型的参数组合？
风险边界：涉及费用、上传、远端 API、JSON 输出或排障过滤时，示例是否展示了必要确认参数和边界参数？
```

按三问决定示例数量：

- 顶层 `-h` 只保留入口级示例，优先选择最高频、最低前置条件、最能代表职责边界的命令。
- 子命令 `-h` 至少提供 1 条常用示例；有 `--json`、过滤条件、确认参数、远端环境、批量输入或输出文件的子命令，提供 2 到 3 条常用示例。
- 高风险或高复杂度子命令可以超过 3 条示例，但必须按场景分组，例如本地、JSON、批量、远端、下载或上传。
- 顶层示例可以覆盖多个子命令，但不要为了“公平”机械穷举所有子命令；不常用命令由命令说明和子命令 `-h` 承接。
- 子命令示例应展示该命令自身的关键参数组合，不要只重复顶层最短示例。
- README 只维护这些选择规则，不维护完整示例集合；示例正文以脚本 help 为准。

## 示例放置

示例应遵循这些规则：

- 示例必须可复制粘贴；占位符只用于必须由用户提供的值，例如 `<job_id>`，并且周围参数组合应保持完整。
- 顶层示例只展示最常用、低歧义的入口路径。
- 子命令示例可以展示多个典型变体。
