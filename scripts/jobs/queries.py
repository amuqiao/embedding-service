    ...
    *,
    ...
    return _fetch_all(
        ...
        SELECT
          ...
          END AS duration
        FROM jobs j
        LEFT JOIN job_attempts a ON a.id = j.active_attempt_id
...
def get_job(conn: connection, job_id: str) -> dict | None:
    return _fetch_one(conn, "SELECT * FROM jobs WHERE id = %(job_id)s AND deleted_at IS NULL", {"job_id": job_id})
...
def attempts(conn: connection, job_id: str) -> list[dict]:
    return _fetch_all(
        conn,
        """
        SELECT a.id::text, a.job_id::text, a.attempt_no, a.status, a.published_at,
               a.dispatch_attempts, a.next_dispatch_at, a.last_dispatch_error,
...
def callbacks(conn: connection, job_id: str) -> list[dict]:
    return _fetch_all(
        ...
        SELECT c.id::text, c.job_id::text, c.event_id::text, c.event_type,
               c.status, c.delivery_attempt, c.next_attempt_at,
...
def stuck(conn: connection, *, older_than: timedelta, limit: int) -> list[dict]:
    ...
    return _fetch_all(
        ...
        """
        (
          SELECT 'queued_attempt_not_published' AS issue, j.id::text AS job_id, j.status AS job_status, j.job_type,
                 a.id::text AS related_id, a.status AS related_status,
                 a.created_at AS since_at, a.next_dispatch_at,
                 a.last_dispatch_error AS detail
          FROM jobs j
          JOIN job_attempts a ON a.id = j.active_attempt_id
          WHERE j.deleted_at IS NULL
            AND j.status IN ('queued', 'running')
            AND a.status = 'queued'
            AND a.created_at < %(cutoff)s
            AND (a.next_dispatch_at IS NULL OR a.next_dispatch_at <= now())
        )
        UNION ALL
        (
          SELECT 'published_attempt_not_claimed' AS issue, j.id::text AS job_id, j.status AS job_status, j.job_type,
                 a.id::text AS related_id, a.status AS related_status,
                 a.published_at AS since_at, a.next_dispatch_at,
                 a.last_dispatch_error AS detail
          FROM jobs j
          JOIN job_attempts a ON a.id = j.active_attempt_id
          WHERE j.deleted_at IS NULL
        ...
        (
          SELECT 'running_attempt_lease_expired' AS issue, j.id::text AS job_id, j.status AS job_status, j.job_type,
                 a.id::text AS related_id, a.status AS related_status,
                 a.lease_expires_at AS since_at, a.next_dispatch_at,
                 a.error AS detail
          FROM jobs j
          JOIN job_attempts a ON a.id = j.active_attempt_id
          WHERE j.deleted_at IS NULL
        ...
        (
          SELECT 'callback_lease_expired' AS issue, j.id::text AS job_id, j.status AS job_status, j.job_type,
                 ...
                 c.lease_expires_at AS since_at, c.next_attempt_at AS next_dispatch_at,
        ...
        (
          SELECT 'terminal_callback_not_settled' AS issue, j.id::text AS job_id, j.status AS job_status, j.job_type,
                 ...
                 j.finished_at AS since_at, j.callback_next_retry_at AS next_dispatch_at,
