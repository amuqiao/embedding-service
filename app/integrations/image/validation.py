...
def _validate_dimensions(width: int, height: int, *, policy: ImageValidationPolicy) -> None:
    ...
    if width > policy.max_width or height > policy.max_height:
        raise AppError(
            "INPUT_TOO_LARGE",
            "reference image dimensions exceed service limit",
            details={
    ...
    if pixels > policy.max_pixels:
        raise AppError(
            "INPUT_TOO_LARGE",
            "reference image pixel count exceeds service limit",
